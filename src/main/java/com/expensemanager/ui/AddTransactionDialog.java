...
public class AddTransactionDialog extends JDialog {
    ...
    private JPanel createCategoryItem(Category c) {
        ...
        // 🌟 TRẢ VỀ NGUYÊN BẢN: Sử dụng setFont gốc của bạn thay vì dùng HTML bộc phát lỗi font
    ...
    private void saveTransaction() {
        try {
            ...
            if (userId == null) { JOptionPane.showMessageDialog(this, "Bạn chưa đăng nhập!"); return; }
            double amount = Double.parseDouble(txtAmount.getText().trim());
            if (amount <= 0) { JOptionPane.showMessageDialog(this, "Số tiền phải lớn hơn 0!"); return; }
            if (selectedCategory == null) { JOptionPane.showMessageDialog(this, "Vui lòng chọn một danh mục!"); return; }
    ...
    private JButton createTypeButton(String text, boolean active) { JButton b = new JButton(text); b.setFont(new Font("Segoe UI", Font.BOLD, 14)); b.setFocusPainted(false); b.setBackground(active ? (text.contains("CHI") ? new Color(244, 67, 54) : new Color(76, 175, 80)) : SURFACE_COLOR); b.setForeground(Color.WHITE); b.setBorder(BorderFactory.createEmptyBorder(10, 0, 10, 0)); return b; }
    private JLabel createLabel(String text) { JLabel l = new JLabel(text); l.setForeground(new Color(150, 150, 150)); l.setFont(new Font("Segoe UI", Font.BOLD, 13)); return l; }
    private JButton createArrowButton(String text) { JButton btn = new JButton(text); btn.setFont(new Font("Segoe UI", Font.BOLD, 12)); btn.setForeground(TEXT_PRIMARY); btn.setBackground(SURFACE_COLOR); btn.setBorder(BorderFactory.createLineBorder(new Color(55, 55, 55), 1)); btn.setFocusPainted(false); btn.setPreferredSize(new Dimension(32, 26)); btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); return btn; }
    private void styleTextField(JTextField tf, String placeholder) { tf.setBackground(SURFACE_COLOR); tf.setForeground(TEXT_PRIMARY); tf.setCaretColor(ACCENT_YELLOW); tf.setFont(new Font("Segoe UI", Font.PLAIN, 16)); tf.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(new Color(50, 50, 50)), BorderFactory.createEmptyBorder(10, 12, 10, 12))); }
}