...
public class FinanceService extends Subject {
    ...
    private Map<String, Category> categoryMap; //bảng băm, để tra cuu theo ID nhanh
    ...
    private void loadInitialData() {
        try {
            List<Category> categories = DatabaseUtil.getAllCategories(); //lấy từ Database
            if (categories != null) {
                for (Category c : categories) {
                    if (c != null) categoryMap.put(c.getId(), c);
                } // in ra console nhung van chay
            }
        } catch (Exception e) {
        ...
        try {
            List<Transaction> saved = JsonUtil.loadFromJson(JSON_FILE_PATH); //tải danh sách từ transaction.json
            transactionList = saved != null ? saved : new ArrayList<>();//lấy nếu nếu ko rỗng or tạo thư mục rỗng
    ...
    public void syncFromDatabase() {
        String userId = SessionManager.getCurrentUserId();// lấy userID từ SesionManager
        if (userId == null) return;

        List<Transaction> dbTransactions = DatabaseUtil.getAllTransactions(userId); //gọi từ Database
        if (dbTransactions != null) {
            this.transactionList = dbTransactions; //gán transactionList
            saveToFile(); //ghi đè lên file JSON
    ...
    public void addCategory(Category category) {
        try {
            DatabaseUtil.insertCategory(category);//thêm danh mục vào database
            categoryMap.put(category.getId(), category);//nếu thành công, thêm vào categoryMap để lần sau tìm nhanh hơn
    ...
    public void addTransaction(Transaction transaction) {
        ...
        try {
            DatabaseUtil.insertTransaction(transaction, userId); // lưu vào MySQL
            transactionList.add(transaction);// thêm vào RAM (transactionList)
            saveToFile();
    ...
    public void updateTransaction(Transaction transaction) {
        try {
            DatabaseUtil.updateTransaction(transaction); //tìm gd cũ trong RAM, thay bằng gd mới
            ...
            saveToFile(); //ghi vào JSON
            notifyObservers(EventType.TRANSACTION_UPDATED, transaction); //tb
        } catch (Exception e) {
    ...
    public void deleteTransaction(String transactionId) {
        try {
            DatabaseUtil.deleteTransaction(transactionId); //xoá khỏi database
            transactionList.removeIf(t -> t.getId().equals(transactionId));
            saveToFile();// ghi JSON
            notifyObservers(EventType.TRANSACTION_DELETED, transactionId);//tb
        } catch (Exception e) {
    ...
    private void saveToFile() {
        try {
            JsonUtil.saveToJson(transactionList, JSON_FILE_PATH); //ghi RAM ra JSON
        } catch (DataLoadException e) {
